...
describe('AdminService', () => {
  ...
  describe('getDashboardMetrics', () => {
    it('returns aggregated dashboard metrics', async () => {
      ...
      vi.mocked(prisma.order.aggregate).mockResolvedValueOnce({
        _sum: { totalCents: 5000000 },
      } as any);
      vi.mocked(prisma.order.aggregate).mockResolvedValueOnce({
        _sum: { totalCents: 4500000 },
      } as any);
  ...
  describe('updateOrderStatus', () => {
    it('throws NotFoundError for non-existent order', async () => {
      ...
      await expect(AdminService.updateOrderStatus('nonexistent', 'CONFIRMED')).rejects.toThrow(
        NotFoundError,
      );
    });
    ...
    it('throws BadRequestError for invalid transition', async () => {
      ...
      await expect(AdminService.updateOrderStatus('ord_1', 'PENDING')).rejects.toThrow(
        BadRequestError,
      );
    });
  ...
  describe('updateCategory', () => {
    it('throws NotFoundError for non-existent category', async () => {
      ...
      await expect(AdminService.updateCategory('nonexistent', { name: 'New' })).rejects.toThrow(
        NotFoundError,
      );
    });
    ...
    it('throws ConflictError for duplicate slug', async () => {
      ...
      await expect(AdminService.updateCategory('cat_1', { slug: 'new' })).rejects.toThrow(
        ConflictError,
      );
    });
  ...
  describe('getAllCustomers', () => {
    it('returns paginated customers with stats', async () => {
      ...
      vi.mocked(prisma.order.groupBy).mockResolvedValue([
        { userId: 'u1', _sum: { totalCents: 5000 }, _count: 2 },
      ] as any);
  ...
  describe('createCoupon', () => {
    it('throws ConflictError for duplicate code', async () => {
      ...
      await expect(
        AdminService.createCoupon({ code: 'SAVE10', type: 'PERCENTAGE', value: 10 }),
    ...
    it('throws BadRequestError for invalid percentage', async () => {
      ...
      await expect(
        AdminService.createCoupon({ code: 'SAVE10', type: 'PERCENTAGE', value: 0 }),
      ).rejects.toThrow(BadRequestError);
      vi.mocked(prisma.coupon.findUnique).mockResolvedValueOnce(null);
      await expect(
        AdminService.createCoupon({ code: 'SAVE10', type: 'PERCENTAGE', value: 101 }),
      ).rejects.toThrow(BadRequestError);
    });
    ...
    it('throws BadRequestError for invalid fixed value', async () => {
      ...
      await expect(
        AdminService.createCoupon({ code: 'SAVE10', type: 'FIXED', value: 0 }),
      ).rejects.toThrow(BadRequestError);
    });
  ...
  describe('toggleCouponActive', () => {
    it('throws NotFoundError for non-existent coupon', async () => {
      ...
      await expect(AdminService.toggleCouponActive('nonexistent', false)).rejects.toThrow(
        NotFoundError,
      );
    });
